...
@section('content')
    ...
    <div class="bg-slate-700 h-screen p-10">
        ...
        <form class="pl-[8rem] pt-8" action="{{ route('password.change',$user) }}" method="POST" class="grid mt-4">
            @csrf

            <div class="grid">
                <label for="email" class="text-neutral-300 font-bold">Enter Reset code</label>
                <input class="w-4/12 text-neutral-200 rounded bg-transparent border-2" type="text" name="code"
                    id="email">
                @error('code')
